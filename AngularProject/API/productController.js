...
exports.productPut = async function (req, res) {
    try {
        db = await MongoClient.connect(url);
        let dbo = db.db("products");
        await dbo.collection("products").updateOne({_id: new mongodb.ObjectId(req.params.id)}, {$set: {name : req.body.name, image : req.body.image, description: req.body.description}});
        res.status(200).send();
    } catch (err) {
        console.log(err);
        res.status(500).json({ message: err });
